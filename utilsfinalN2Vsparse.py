...
node2vec = Node2Vec(smaller_graph, dimensions=1, walk_length=15, num_walks=30, p=1, q=0.5, workers=1)
...
def reward_function(success, max_round, state):
    ...
    assign_probs = torch.softmax(model.assign_net(state_tensor), dim=-1)  # [K]
