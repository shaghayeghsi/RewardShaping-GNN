...
class DQNAgent:
    ...
    def _build_model(self):
        """Builds and returns dueling model/graph of neural network."""

        input_layer = Input(shape=(self.state_size,))
        
        # Shared layers
        shared_layer_1 = Dense(self.hidden_size, activation='relu')(input_layer)
        
        # Advantage stream
        adv_stream = Dense(self.hidden_size, activation='relu')(shared_layer_1)
        adv_output = Dense(self.num_actions, activation='linear')(adv_stream)
        
        # Value stream
        val_stream = Dense(self.hidden_size, activation='relu')(shared_layer_1)
        val_output = Dense(1, activation='linear')(val_stream)
        
        # Combine advantage and value streams using Lambda layer
        dueling_output = Lambda(lambda x: x[1] + (x[0] - K.mean(x[0], axis=1, keepdims=True)), output_shape=(self.num_actions,))([adv_output, val_output])
